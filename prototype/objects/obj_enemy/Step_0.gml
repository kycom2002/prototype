...
else if(collision_line(x,y,obj_witch.x,y,obj_wall,false,false))
{
	sprite_index = sp_enemy;
}
if(distance_to_object(obj_witch) <= guard-cons)
{
	sprite_index = sp_enemy_follow;
	hsp = ghsp;
	if(distance_to_object(obj_witch) < aggroRange-cons)
{
	if(!collision_line(x,y,obj_witch.x,y,obj_wall,false,false))
	{
		if(!obj_witch.hide_ability = true)
		{
			mp_potential_step(obj_witch.x,obj_witch.y,hsp,false);
			sprite_index = sp_enemy_follow;
			
		}
		else if(obj_witch.hide_ability = true)
		{
			sprite_index = sp_enemy;
